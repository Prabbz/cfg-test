function refreshExtensionSettingsTableElement(tableElement, settings, secureSettings, dontValidate, onlyRowsOfType)
{
    if (!tableElement)
        return;
    ...
    for (var i = 0; i < rows.length; ++i) {
        if (onlyRowsOfType && rows[i].interfaceItem["Type"] !== onlyRowsOfType)
            continue;
...
function createExtensionSettingsTableRows(settingsInterfaceItems, settings, secureSettings, settingChangedCallback, parentInterfaceItem, rootSettings, rootSecureSettings, dontValidate)
{
    ...
    function validateInput(interfaceItem, value, control)
    {
        ...
        if (result) {
            parentCell.classList.add("invalid");
            ...
            parentCell.classList.remove("invalid");
            if (parentCell.errorElement) {
    ...
    function convertValue(interfaceItem, value)
    {
        ...
        case "boolean":
            return value === "true";
        case "number":
            // If the type of the value is actually number, then the result of parseFloat should not
            // be NaN. However, the Extension Builder UI could be expecting a number in a text field.
            // In that case, don't trust the user to enter in something that is a number. Replace
            // any non-numbers with the empty string.
    ...
    function performTableActionOnTableControl(actionFunction, table, settings, secureSettings)
    {
        ...
        for (var i = 0; i < tableBodies.length; ++i) {
            ...
            if (tableBody.classList.contains("creation"))
                continue;
